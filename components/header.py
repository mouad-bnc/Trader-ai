...
def render_header(active: str, items: list[str]) -> None:
    ...
    st.markdown(f"<div class='topbar'><div class='brand'><span><b>✦</b> {APP_NAME}</span><small>{APP_SUBTITLE} · v{APP_VERSION}</small></div><div class='nav'>{links}</div><div class='muted'>{now_utc_label()}</div></div>", unsafe_allow_html=True)
